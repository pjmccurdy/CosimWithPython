...
public class RandNumGenBase extends SynchronizedFederate {
    private SubscribedInteractionFilter _subscribedInteractionFilter =
        new SubscribedInteractionFilter();

    // constructor
    public RandNumGenBase(FederateConfig config) throws Exception {
        super(config);
        super.createLRC();
        ...
        // interaction pubsub
        ReceiveNum.publish(getLRC());
        SendNum.subscribe(getLRC());
    ...
    public ReceiveNum create_ReceiveNum() {
        ReceiveNum interaction = new ReceiveNum();
